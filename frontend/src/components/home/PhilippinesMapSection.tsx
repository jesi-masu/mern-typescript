...
const projectLocations: ProjectLocation[] = [
  // Luzon
  { id: 1, name: 'Claveria', region: 'Northern Luzon', type: 'residential', x: 180, y: 60, projects: 6, icon: Home },
  { id: 2, name: 'Tuguegarao', region: 'Northern Luzon', type: 'office', x: 195, y: 50, projects: 8, icon: Building },
  { id: 11, name: 'Manila', region: 'Northern Luzon', type: 'office', x: 170, y: 150, projects: 25, icon: Building },
  { id: 12, name: 'Baguio', region: 'Northern Luzon', type: 'residential', x: 160, y: 100, projects: 7, icon: Home },
  
  // Central Visayas
  { id: 3, name: 'Cebu', region: 'Central Visayas', type: 'office', x: 210, y: 220, projects: 12, icon: Building },
  { id: 4, name: 'Bohol', region: 'Central Visayas', type: 'residential', x: 235, y: 235, projects: 5, icon: Home },
  { id: 13, name: 'Iloilo', region: 'Central Visayas', type: 'industrial', x: 175, y: 210, projects: 9, icon: Factory },
  { id: 14, name: 'Boracay', region: 'Central Visayas', type: 'residential', x: 160, y: 190, projects: 3, icon: Home },
  
  // Mindanao
  { id: 5, name: 'Cagayan de Oro', region: 'Mindanao', type: 'industrial', x: 210, y: 270, projects: 10, icon: Factory }, // <--- Target for special pulse
  { id: 6, name: 'Iligan', region: 'Mindanao', type: 'industrial', x: 200, y: 265, projects: 7, icon: Factory },
  { id: 7, name: 'Agusan', region: 'Mindanao', type: 'residential', x: 250, y: 285, projects: 4, icon: Home },
  { id: 8, name: 'Bukidnon', region: 'Mindanao', type: 'residential', x: 220, y: 290, projects: 6, icon: Home },
  { id: 9, name: 'General Santos', region: 'Mindanao', type: 'office', x: 210, y: 350, projects: 8, icon: Building },
  { id: 10, name: 'Zamboanga', region: 'Mindanao', type: 'industrial', x: 180, y: 320, projects: 9, icon: Factory },
  { id: 15, name: 'Davao', region: 'Mindanao', type: 'office', x: 240, y: 320, projects: 15, icon: Building },
];
...
const PhilippinesMapSection: React.FC = () => {
  ...
  const getProjectTypeColorClass = (type: string) => projectTypeColors[type as keyof typeof projectTypeColors] || 'text-gray-500';
  const getProjectTypeBgColorClass = (type: string) => projectTypeBgColors[type as keyof typeof projectTypeBgColors] || 'bg-gray-500';
  const getRegionColorClass = (region: string) => regionColors[region as keyof typeof regionColors] || 'text-gray-500';
  ...
  const cagayanDeOro = projectLocations.find(loc => loc.name === 'Cagayan de Oro');
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-16">
          ...
          <p className="text-lg text-gray-700 max-w-3xl mx-auto leading-relaxed">
            Explore our diverse portfolio of completed prefabricated projects, showcasing our nationwide presence and commitment to quality, efficiency, and tailored solutions.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className="order-2 lg:order-1 space-y-8">
            <p className="text-gray-700 text-lg leading-relaxed">
              From the vibrant urban centers to the serene rural landscapes, Camco Prefab has left its mark. Our projects range from modular homes providing rapid housing solutions, to cutting-edge prefab offices fostering modern workspaces, and robust industrial facilities supporting vital industries. Each completed structure is a testament to our adaptability and precision, engineered to meet the unique demands of clients across the archipelago.
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              <div className="text-center p-6 bg-white rounded-2xl shadow-xl border border-green-200 transform transition-all duration-300 hover:scale-105 hover:shadow-2xl">
                ...
                <div className="text-md text-gray-700 font-semibold">Total Projects</div>
              </div>
              <div className="text-center p-6 bg-white rounded-2xl shadow-xl border border-blue-200 transform transition-all duration-300 hover:scale-105 hover:shadow-2xl">
                ...
                <div className="text-md text-gray-700 font-semibold">Cities Covered</div>
              </div>
              <div className="text-center p-6 bg-white rounded-2xl shadow-xl border border-purple-200 transform transition-all duration-300 hover:scale-105 hover:shadow-2xl">
                ...
                <div className="text-md text-gray-700 font-semibold">Key Regions</div>
              </div>
            </div>

            {/* Regional Overview - Refined List */}
            <div className="bg-white rounded-2xl p-6 shadow-xl border border-gray-100">
              <h3 className="font-bold text-xl text-gray-900 mb-4">Regional Distribution</h3>
              <div className="space-y-3">
                {Object.entries(regionalProjectCounts).map(([region, count]) => (
                  <div key={region} className="flex items-center justify-between group cursor-default">
                    <div className="flex items-center gap-3">
                      <div className={`w-4 h-4 rounded-full ${getRegionColorClass(region).replace('text-', 'bg-')} shadow-sm group-hover:scale-110 transition-transform`}></div>
                      <span className="text-lg text-gray-700 font-medium group-hover:text-gray-900 transition-colors">{region}</span>
                    </div>
                    <span className="text-lg font-bold text-gray-800 group-hover:text-blue-600 transition-colors">{count} projects</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="flex justify-center order-1 lg:order-2 relative">
            <div className="relative w-full max-w-lg p-4 bg-white/50 rounded-3xl shadow-2xl backdrop-blur-md border border-gray-200">
              ...
              <svg viewBox="0 0 300 400" className="w-full h-auto drop-shadow-2xl">
                <defs>
                  {/* Water gradient */}
                  ...
                  {/* Luzon gradient */}
                  <linearGradient id="luzonGradient" x1="0%" y1="0%" x2="100%" y2="100%">
                  ...
                  {/* Visayas gradient */}
                  <linearGradient id="visayasGradient" x1="0%" y1="0%" x2="100%" y2="100%">
                  ...
                  {/* Mindanao gradient */}
                  <linearGradient id="mindanaoGradient" x1="0%" y1="0%" x2="100%" y2="100%">
                  ...
                  <pattern id="waves" patternUnits="userSpaceOnUse" width="40" height="40" patternTransform="rotate(45)">
                ...
                <rect width="300" height="400" fill="url(#waves)" opacity="0.4" />
                ...
                <path
                  ...
                  filter={`url(#dropshadow) ${hoveredRegion === 'Northern Luzon' ? 'url(#glow)' : ''}`.trim()}
                  className={`transition-all duration-500 hover:brightness-110 cursor-pointer ${
                    hoveredRegion === 'Northern Luzon' ? 'brightness-120 scale-[1.02]' : ''
                  }`}
                ...
                <g>
                  ...
                  <path
                    ...
                    strokeWidth="1.5"
                    // Fix: Construct filter string more robustly
                    filter={`url(#dropshadow) ${hoveredRegion === 'Central Visayas' ? 'url(#glow)' : ''}`.trim()}
                    className={`transition-all duration-500 hover:brightness-110 cursor-pointer ${
                      hoveredRegion === 'Central Visayas' ? 'brightness-120 scale-[1.02]' : ''
                    }`}
                ...
                <path
                  ...
                  strokeWidth="2"
                  // Fix: Construct filter string more robustly
                  filter={`url(#dropshadow) ${hoveredRegion === 'Mindanao' ? 'url(#glow)' : ''}`.trim()}
                  className={`transition-all duration-500 hover:brightness-110 cursor-pointer ${
                    hoveredRegion === 'Mindanao' ? 'brightness-120 scale-[1.02]' : ''
                  }`}
                  ...
                  return (
                    ...
                    >
                      ...
                      <circle
                        ...
                        fill="none"
                        stroke={getProjectTypeBgColorClass(location.type).replace('bg-', '')}
                        strokeWidth="2"
                        opacity="0.4"
                        className={`transition-all duration-300 ${isActive ? 'animate-ping-strong' : 'animate-ping-subtle'}`}
                      />
                      ...
                      <circle
                        ...
                        fill={getProjectTypeBgColorClass(location.type).replace('bg-', '')}
                        stroke="white"
                        strokeWidth="2"
                        filter="url(#glow)"
                        className={`transition-all duration-300 ${isActive ? 'scale-125' : 'scale-100'} drop-shadow-lg`}
                      />

                      {/* Icon inside the marker */}
                      <g 
                        transform={`translate(${location.x - 6}, ${location.y - 6})`} 
                        fill="white"
                      ...
                      <circle
                        ...
                        strokeWidth="1.5"
                        filter="url(#dropshadow)"
                        className={`transition-all duration-300 ${isActive ? 'scale-110' : 'scale-100'} origin-center`}
                      />
                      <text
                        x={location.x + 8}
                        y={location.y - 8}
                      ...
                      {activeLocation === location.id && (
                        <g className="animate-fade-in-up">
                          ...
                          {/* Triangle pointer */}
                          <polygon 
                            points={`${location.x},${location.y - 5} ${location.x - 5},${location.y + 15} ${location.x + 5},${location.y + 15}`} 
                            fill="white" 
                            stroke={getProjectTypeBgColorClass(location.type).replace('bg-', '')}
                            strokeWidth="1.5"
                            className="drop-shadow-sm"
                            transform={`translate(0, ${location.y - 5}) rotate(180 ${location.x} ${location.y + 10})`} // Rotate to point downwards from tooltip
                          />
                          <text
                            x={location.x}
                ...
                {cagayanDeOro && (
                    <circle
                        ...
                        stroke={getProjectTypeBgColorClass(cagayanDeOro.type).replace('bg-', '')} // Match project type color
                        strokeWidth="2"
              ...
              <div className="absolute -bottom-6 -right-6 bg-white p-5 rounded-3xl shadow-2xl border border-gray-100 backdrop-blur-sm bg-opacity-90">
                <div className="text-base font-bold text-gray-800 mb-4">Map Legend</div>
                
                {/* Project Types */}
                <div className="mb-4">
                  <h4 className="font-semibold text-gray-700 text-sm mb-2">Project Types:</h4>
                  <div className="space-y-2">
                    {Object.entries(projectTypeColors).map(([type, colorClass]) => (
                      <div key={type} className="flex items-center gap-3 text-sm">
                        <div className={`w-4 h-4 rounded-full ${colorClass.replace('text-', 'bg-')} shadow-sm`}></div>
                ...
                {/* Regions */}
                <div>
                  <h4 className="font-semibold text-gray-700 text-sm mb-2">Regions:</h4>
                  <div className="space-y-2">
