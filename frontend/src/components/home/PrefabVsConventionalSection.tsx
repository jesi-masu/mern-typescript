...
import { ArrowRight, Clock, DollarSign, Zap, Wrench, CheckCircle, AlertCircle, X } from "lucide-react";
...
const PrefabVsConventionalSection = () => {
  ...
  const [activeColumn, setActiveColumn] = useState<'prefab' | 'conventional' | null>(null);
      ...
      <div className="container relative z-10 max-w-7xl mx-auto px-6">
          ...
          >
            <Card
              className={`bg-gradient-to-br from-emerald-900/20 to-green-800/20 border-emerald-500/20 rounded-2xl mb-10 overflow-hidden transition-all duration-500 backdrop-blur-sm ${
                activeColumn === 'prefab' ? 'scale-[1.02] shadow-2xl shadow-emerald-500/10 border-emerald-400/40 bg-gradient-to-br from-emerald-900/30 to-green-800/30' : ''
              }`}
            >
              <CardContent className="p-8 h-80 flex flex-col justify-center items-center relative">
                ...
                <div
                  ...
                  onClick={() => openImageModal("/photos/Technicaldata/prefab2.jpg", "Modern Prefab Construction")}
                >
                  <div className={`absolute inset-0 bg-gradient-to-t from-emerald-500/5 via-transparent to-green-400/5 rounded-xl transition-all duration-500 ${
                    activeColumn === 'prefab' ? 'opacity-100 scale-105' : 'opacity-0'
                  }`}></div>

                  <img
                    src="/photos/Technicaldata/prefab2.jpg"
                    ...
                    className={`max-w-full max-h-full object-contain rounded-lg shadow-lg transition-all duration-500 ${
                      activeColumn === 'prefab' ? 'scale-110 brightness-110 shadow-emerald-500/20' : 'scale-100'
                    }`}
                  />

                  {/* Image overlay effects */}
                  <div className={`absolute inset-0 border-2 border-emerald-400/0 rounded-lg transition-all duration-500 ${
            ...
            <div className="space-y-8">
              ...
              {prefabFeatures.map((feature, index) => (
                ...
                >
                  <div className={`bg-gradient-to-br from-emerald-500 to-green-600 p-3 rounded-xl mr-5 flex-shrink-0 mt-1 shadow-lg transition-all duration-300 group-hover:scale-110 group-hover:shadow-emerald-500/25`}>
                    {feature.icon}
                  </div>
          ...
          >
            <Card
              className={`bg-gradient-to-br from-orange-900/20 to-amber-800/20 border-orange-500/20 rounded-2xl mb-10 overflow-hidden transition-all duration-500 backdrop-blur-sm ${
                activeColumn === 'conventional' ? 'scale-[1.02] shadow-2xl shadow-orange-500/10 border-orange-400/40 bg-gradient-to-br from-orange-900/30 to-amber-800/30' : ''
              }`}
            >
              <CardContent className="p-8 h-80 flex flex-col justify-center items-center relative">
                <div className="absolute top-6 right-6 z-10">
                ...
                <div
                  className="relative w-full h-full flex items-center justify-center group cursor-pointer"
                  onClick={() => openImageModal("/photos/Technicaldata/conventional.jpg", "Traditional Conventional Construction")}
                >
                  <div className={`absolute inset-0 bg-gradient-to-t from-orange-500/5 via-transparent to-amber-400/5 rounded-xl transition-all duration-500 ${
                    activeColumn === 'conventional' ? 'opacity-100 scale-105' : 'opacity-0'
                  }`}></div>
                  ...
                  <img
                    ...
                    className={`max-w-full max-h-full object-contain rounded-lg shadow-lg transition-all duration-500 ${
                      activeColumn === 'conventional' ? 'scale-110 brightness-110 shadow-orange-500/20' : 'scale-100 grayscale-[0.2]'
                    }`}
                  />

                  {/* Image overlay effects */}
                  <div className={`absolute inset-0 border-2 border-orange-400/0 rounded-lg transition-all duration-500 ${
                    activeColumn === 'conventional' ? 'border-orange-400/30' : ''
                  }`}></div>
            ...
            <div className="space-y-8">
              ...
              {conventionalFeatures.map((feature, index) => (
                ...
                >
                  <div className={`bg-gradient-to-br from-orange-500 to-amber-600 p-3 rounded-xl mr-5 flex-shrink-0 mt-1 shadow-lg transition-all duration-300 group-hover:scale-110 group-hover:shadow-orange-500/25`}>
                    {feature.icon}
                  </div>
                  <div className="flex-1">
        ...
        <div className="fixed bottom-8 left-8 z-50">
          {activeColumn && (
            ...
            >
              {activeColumn === 'prefab' ? '⚡ Fast & Efficient Solution' : '⚠ Traditional Approach'}
            </div>
          )}
        </div>
            ...
            {[...Array(4)].map((_, i) => (
              <div
                ...
                className={`w-3 h-3 rounded-full transition-all duration-500 ${
                  animatedFeatures.includes(i) || animatedFeatures.includes(i + 4)
